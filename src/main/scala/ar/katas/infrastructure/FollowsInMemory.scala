...
object FollowsInMemory {
  val make: IO[Follows] =
    IO.ref(Map.empty[FollowerId, List[FolloweeId]]).map(database => {
      new Follows {
        override def persistFollowing(idFollower: FollowerId, idFollowee: FolloweeId): IO[Unit] =
          database.update(m => {
            val updatedList = idFollowee :: m.get(idFollower).getOrElse(List.empty)
